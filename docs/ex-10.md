...
| $LB$ (constructive heuristic) | $z(\mathit{MILP})$ | $z(\mathit{LP})$ | $z(\mathit{LP}^+)$ | $UB$ (dual by hand) | heuristic gap |
|---:|---:|---:|---:|---:|---:|
...
!!! tip "Why the relaxation is so weak here"
    $z(\mathit{LP}) = 5200$ against $z(\mathit{MILP}) = 2500$: more than double.
